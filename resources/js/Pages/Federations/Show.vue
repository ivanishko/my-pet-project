                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="mb-8">
                            ...
                            <div v-if="federation.website || federation.email || federation.phone" class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div v-if="federation.website">
                                    <p class="text-gray-500">Веб-сайт:</p>
                                    <a :href="federation.website" target="_blank" class="text-blue-500 hover:underline">
                                        {{ federation.website }}
                                    </a>
                                </div>
                                <div v-if="federation.email">
                                    <p class="text-gray-500">Email:</p>
                                    <a :href="`mailto:${federation.email}`" class="text-blue-500 hover:underline">
                                        {{ federation.email }}
                                    </a>
                                </div>
                                <div v-if="federation.phone">
                                ...
                                >
                                    <div class="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p class="text-gray-600 mt-1">{{ tournament.description || 'Нет описания' }}</p>
                                            <div class="mt-2 text-sm text-gray-500">
                                                <p>Место: {{ tournament.location }}</p>
                                                <p>Тип: {{
                                                    tournament.type === 'individual' ? 'Индивидуальный' :
                                                    tournament.type === 'team' ? 'Командный' :
                                                    'Смешанный'
                                                    }}</p>
                                                <p>Статус: {{
                                                    tournament.status === 'active' ? 'Активный' :
                                                    tournament.status === 'inactive' ? 'Неактивный' :
                                                    'Завершен'
                                                    }}</p>
                                            </div>
                                        </div>
