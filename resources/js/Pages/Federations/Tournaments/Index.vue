                            ...
                            >
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-gray-600">{{ tournament.discipline }}</p>
                                        <!-- Используем seasons_count вместо seasons.length -->
                                        <div v-if="tournament.seasons_count > 0" class="mt-2 text-sm">
                                            <p>Количество сезонов: {{ tournament.seasons_count }}</p>
                                        </div>
                                    </div>
                                    <div class="flex space-x-2">
                                        <Link
                                            :href="route('tournaments.seasons.index', tournament.id)"
                                            class="text-green-500 hover:text-green-700 px-3 py-1 border border-green-500 rounded hover:bg-green-50"
                                        >
                                            Сезоны ({{ tournament.seasons_count || 0 }})
                                        </Link>
                                        <Link
